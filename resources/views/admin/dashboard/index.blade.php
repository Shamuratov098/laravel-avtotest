...
@section('content')
    ...
    <div style="display:grid; grid-template-columns:repeat(4,1fr); gap:24px; margin-bottom:24px;">
        @foreach($cards as $card)
            ...
            <div style="background:#fff; border-radius:14px; padding:24px; box-shadow:0 1px 2px rgba(0,0,0,.04); border:1px solid #F3F4F6;">

                {{-- Icon --}}
                <div style="width:52px; height:52px; background:{{ $card['icon_bg'] }}; border-radius:14px;
                            display:flex; align-items:center; justify-content:center; margin-bottom:18px;">
                    <svg width="26" height="26" fill="none" stroke="{{ $card['icon_color'] }}" stroke-width="2" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" d="{{ $card['icon'] }}"/>
                    </svg>
                </div>

                {{-- Number + Label --}}
                <p style="font-size:30px; font-weight:800; color:#1C2434; margin:0 0 4px; line-height:1.1; letter-spacing:-0.02em;">
                    {{ number_format($card['value']) }}
                </p>
                <p style="font-size:13px; color:#637381; margin:0 0 14px;">{{ $card['label'] }}</p>

                {{-- Trend va caption (NextAdmin uslubi) --}}
                @if($card['secondary'])
                    <div style="display:flex; align-items:center; gap:5px; font-size:12px; line-height:1;">
                        <span style="font-weight:700; color:#10B981;">+{{ number_format($card['secondary']['value']) }}</span>
                        <span style="color:#8899A8; font-weight:500;">{{ $card['secondary']['label'] }}</span>
                    </div>
                @else
                    <span class="trend-pill {{ $trendClass($trend['direction']) }}">
                        <span class="trend-arrow">{{ $trendIcon($trend['direction']) }}</span>
                        <span>{{ $trend['percent'] }}%</span>
                        <span class="trend-caption">so'nggi 7 kun</span>
                    </span>
                @endif
            </div>
        @endforeach
    ...
    {{-- ============== MAIN CHART ============== --}}
    <div style="background:#fff; border-radius:14px; padding:24px; box-shadow:0 1px 2px rgba(0,0,0,.04); border:1px solid #F3F4F6; margin-bottom:24px;">
        <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:20px;">
            <div>
                <h3 style="font-size:16px; font-weight:700; color:#1C2434; margin:0 0 2px; letter-spacing:-0.01em;">Test Bajarishlar</h3>
                <p style="font-size:12px; color:#637381; margin:0;" id="chart-subtitle">Oxirgi 30 kun bo'yicha statistika</p>
            </div>
            <div class="seg-group">
                <button onclick="updateChart('daily')" id="btn-daily" class="seg-btn is-active">Kunlik</button>
                <button onclick="updateChart('monthly')" id="btn-monthly" class="seg-btn">Oylik</button>
                <button onclick="updateChart('yearly')" id="btn-yearly" class="seg-btn">Yillik</button>
            </div>
        </div>
    ...
    <div style="display:grid; grid-template-columns:2fr 1fr; gap:24px; margin-bottom:24px;">
        ...
        <div style="background:#fff; border-radius:14px; padding:24px; box-shadow:0 1px 2px rgba(0,0,0,.04); border:1px solid #F3F4F6;">
            <div style="margin-bottom:18px;">
                <h3 style="font-size:16px; font-weight:700; color:#1C2434; margin:0 0 2px; letter-spacing:-0.01em;">Top Kategoriyalar</h3>
        ...
        <div style="background:#fff; border-radius:14px; padding:24px; box-shadow:0 1px 2px rgba(0,0,0,.04); border:1px solid #F3F4F6;">
            <div style="margin-bottom:18px;">
                <h3 style="font-size:16px; font-weight:700; color:#1C2434; margin:0 0 2px; letter-spacing:-0.01em;">Testdan o'tish ko'rsatgichlari</h3>
            ...
            @else
                <div style="position:relative; width:100%; max-width:200px; margin:0 auto;">
                    ...
                    <div style="position:absolute; top:50%; left:50%; transform:translate(-50%,-50%); text-align:center; pointer-events:none;">
                        <div style="font-size:36px; font-weight:800; color:#1C2434; line-height:1; letter-spacing:-0.02em;">{{ $passRate['percent'] }}%</div>
                    </div>
                </div>
    ...
    <div style="background:#fff; border-radius:14px; padding:24px; box-shadow:0 1px 2px rgba(0,0,0,.04); border:1px solid #F3F4F6;">
        <div style="margin-bottom:18px;">
            <h3 style="font-size:16px; font-weight:700; color:#1C2434; margin:0 0 2px; letter-spacing:-0.01em;">Oxirgi yechilgan testlar</h3>
            ...
            <div>
                @foreach($recentSessions as $s)
                    ...
                    <div class="session-row" style="display:flex; align-items:center; gap:14px; padding:12px 8px; border-radius:8px;">
                        <div style="width:42px; height:42px; min-width:42px; border-radius:10px; background:{{ $av['bg'] }}; display:flex; align-items:center; justify-content:center;">
                            <span style="font-size:15px; font-weight:700; color:{{ $av['fg'] }};">{{ $initial }}</span>
                        </div>

                        <div style="flex:1; min-width:0;">
                            <div style="font-size:14px; font-weight:600; color:#1C2434; margin-bottom:3px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">
                                {{ $s['user_name'] }}
                            </div>
                            <div style="display:flex; align-items:center; gap:8px; font-size:12px; color:#8899A8;">
                                @if($s['is_random'])
                                    <span style="display:inline-flex; align-items:center; padding:2px 8px; border-radius:6px; background:#F3F4F6; color:#637381; font-weight:600; font-size:11px;">Tasodifiy</span>
                                @else
